Evader.extend('utils', {
	...
	view: {
		...
		getImgHtml(imjObj, className) {
			return `<img alt='${imjObj.alt}' class='${imjObj.className || className}' src='${imjObj.src}'>`;
		},
		...
		getBorderWidth(element) {
			return parseInt(getComputedStyle(element).borderWidth);
		},
	...
	game: {
		getMedalType(gameMode, finalScore) {
			const { medalScores } = Evader.config.game[gameMode];

			const medalTypes = Object.keys(medalScores).sort((typeA, typeB) => medalScores[typeB] - medalScores[typeA]);
			const medalIndex = medalTypes.findIndex((medalType) => finalScore > medalScores[medalType]);
