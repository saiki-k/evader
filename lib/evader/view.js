		...
		const renderers = {
			...
			renderGameMeta(
				...
				const {
					finalScore,
					bestScore,
					constants: { isSmallerGameArea, gameAreaDescriptor },
				} = gameState;

				if (['GAME_INIT', 'GAME_OVER'].includes(gameState.updateType)) {
					const bestScoreImg = isSmallerGameArea ? html.sportsMedalImg : html.firstPlaceMedalImg;
					const prevBestScore = bestScore[gameAreaDescriptor];
					const prevFinalScore = finalScore[gameAreaDescriptor];
			...
			renderGameComment(
				...
				if (gameState.updateType === 'GAME_OVER') {
					const finalScore = gameState.finalScore[gameState.constants.gameAreaDescriptor];
					elements.gameComment.innerHTML = `
						<br>You've survived for ${finalScore} seconds.
						<br>${
							finalScore >= gameState.constants.radScore
