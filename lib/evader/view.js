	...
	(function () {
		...
		const eventHandlers = {
			createGameOverlayButtonHandler(
				...
				const activateButtonElement = (buttonElement) => {
					Evader.audio.play('optionSelect');
					buttonElement.classList.add('active');
				};

				const deactivateButtonElement = (buttonElement) => {
				...
				return function (e) {
					...
					if (buttonIsNotActive) {
						return activateButtonElement(buttonElement);
					}

					const pointerEventTargetIsNotButtonSpan =
						['mouse', 'touch', 'pen'].includes(e.pointerType) && e.target.nodeName !== 'SPAN';
					if (pointerEventTargetIsNotButtonSpan) {
						deactivateButtonElement(buttonElement);
						return Evader.audio.play('optionSelect');
					}
