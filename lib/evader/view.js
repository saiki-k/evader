Evader.extend(
	'view',
	(function () {
		const { getImgHtml } = Evader.view.utils;
		const {
			emoji: {
				victoryHand,
				firstPlaceMedal,
		...
		const eventListeners = {
			register(target, event, listener, options = { removeListener: false }) {
			...
			handleHold(e) {
				...
				Evader.game.holdPlayer({ x: e.pageX, y: e.pageY });

				const { register, handleMove, handleRelease } = eventListeners;
				Evader.constants.events.move.forEach((event) => register(document, event, handleMove));
				Evader.constants.events.release.forEach((event) => register(document, event, handleRelease));
			},

			handleMove(e) {
				e.preventDefault();
				Evader.game.movePlayer({ x: e.pageX, y: e.pageY });
			},
			...
			handleRelease(e) {
				...
				Evader.game.releasePlayer({ x: e.pageX, y: e.pageY });
			},
		};
