		...
		return {
			...
			endGame(collisionState) {
				const {
					...
					game: { elapsedTime },
					misc: {
						gameOptions: { isRandomiserEnabled, gameMode, isDarkThemeEnabled, godMode },
				...
				const randomiserEnabledForAllValues = !Object.values(isRandomiserEnabled).some((v) => !v);
				const prevBestScore = bestScore[gameAreaDescriptor][gameMode];
				const isNewBestScore = !prevBestScore || finalScore > prevBestScore;
				const isBestScoreGame = isNewBestScore && randomiserEnabledForAllValues && !godMode.wasToggled;
				...
				gameStateUpdates.push({
					...
					newState: [
						...gameHistory,
						{
							...
							gameAreaDescriptor,
							gameMode,
							randomiserWasEnabled: randomiserEnabledForAllValues,
							godModeWasToggled: godMode.wasToggled,
							darkThemeWasEnabled: isDarkThemeEnabled,
							isBestScoreGame,
						},
					],
				});
