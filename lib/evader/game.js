Evader.extend(
	...
	(function () {
		...
		let gameLoopTimer = null,
			recalculateGameAreaBounds;
		const runGameLoop = () => {
			const { gameTick, enemyPositions, startTime, updateInterval, finalScore } = Evader.state.getGameState();
			const {
				elements,
			 ...
			 ** We recalculate the boundind rect. in such instances to account for proper `isElementAtBounds` calculations.
			 */
			if (gameTick === 0 && !finalScore) {
				recalculateGameAreaBounds = true;
			}

			if (gameTick === 1 && recalculateGameAreaBounds) {
				recalculateGameAreaBounds = false;
				gameStateUpdates.push({
					statePath: 'constants.gameArea.boundingRect',
					newState: getBoundingRect(elements.gameArea),
				});
			}
