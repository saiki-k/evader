Evader.extend(
	...
	(function () {
		...
		const getNewEnemyPosition = (enemy) => {
			const { gameArea, enemySpeeds, enemyPositions, enemyDirectionVectors } = Evader.state.getGameState();
			const { isEnemyAtBounds } = Evader.view;

			const enemySpeed = enemySpeeds[enemy];
			const enemyPosition = enemyPositions[enemy];
			const enemyDirectionVector = enemyDirectionVectors[enemy];

			const enemyAtBounds = isEnemyAtBounds(enemy, gameArea);

			const newDirectionVector = {
				x: (enemyAtBounds.x ? -1 : 1) * enemyDirectionVector.x,
				y: (enemyAtBounds.y ? -1 : 1) * enemyDirectionVector.y,
			};

			const newPosition = {
				x: enemyPosition.x + enemySpeed.x * newDirectionVector.x,
				y: enemyPosition.y + enemySpeed.y * newDirectionVector.y,
			};
